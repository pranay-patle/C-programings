...
#define NUM_ROWS 100// Number of rows in the matrices
#define NUM_COLS 100// Number of columns in the matrices
...
int main() {
    ...
    for (int degree = 2; degree <= 10; degree++) {
        ...
        for (int i = 0; i < NUM_ROWS; i++) {
            for (int j = 0; j < NUM_COLS; j++) {
                errorSum += fabs(resultDoubleMatrix[i][j] - resultFloatMatrix[i][j]);
